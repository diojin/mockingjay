...
1. Index
    ...
    7. 应尽量避免在where子句中对字段进行函数操作, 会导致索引失效, 除非建立函数索引.
    8. 如果在 where 子句中使用参数(采用绑定变量)，也会导致全表扫描?????  
...
```
    ...
    9. leading columns are used when using composite index, unless index skip scan can apply, otherwise composite index is not used 
    10. 对索引列使用OR将造成全表扫描.
    11. Index can’t have more than half of the columns of a table  
